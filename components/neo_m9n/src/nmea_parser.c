...
static void parse_gga(esp_gps_t* esp_gps) {
    ...
    switch (esp_gps->item_num) {
        ...
        case 6: /* Fix status */
            esp_gps->parent.fix =
                (gps_fix_t)strtol(esp_gps->item_str, NULL, 10);
            break;
    ...
    switch (esp_gps->item_num) {
        ...
        default:
            if (esp_gps->item_num >= 4 && esp_gps->item_num <= 19) {
                ...
                uint8_t index;
                uint32_t value;
                index = 4 * (esp_gps->sat_num - 1) +
                        item_num / 4; /* Get array index */
...
static void nmea_parser_task_entry(void* arg) {
    ...
    while (1) {
        if (xQueueReceive(esp_gps->event_queue, &event, pdMS_TO_TICKS(200))) {
            switch (event.type) {
                case UART_DATA:
                    ESP_LOGI(GPS_TAG, "UART data received");
                    break;
                case UART_FIFO_OVF:
...
nmea_parser_handle_t nmea_parser_init(const nmea_parser_config_t* config) {
    ...
    if (uart_driver_install(esp_gps->uart_port,
                            CONFIG_NMEA_PARSER_RING_BUFFER_SIZE, 0,
